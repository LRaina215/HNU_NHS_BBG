...
fn setup_vehicle(
    ...
    for (node, name, &ChildOf(secondary), transform) in node_query {
        ...
        match name.as_str() {
            ...
            "SHOT_DIRECTION" => {
                // 新代码: 手动把 Y 轴 (高度) 降下来
                let mut new_trans = transform.clone();
                // 下降 0.1 米 (根据你的实际情况调整这个数值)
                // 如果是从“上面”发射的，就减小 Y；如果是左右偏，就改 X
                new_trans.translation.y -= 0.6; 
                // new_trans.translation.z += 0.05; // 如果需要前后调整
                
                ent.insert(InfantryLaunchOffset(new_trans));
            }
            "CAM_DIRECTION" => {
		let mut new_trans = transform.clone();
                // 往前推 0.2 米，防止被炮管/装甲板挡住
                // 注意：坐标轴方向取决于模型定义，通常 Z 是前后，Y 是上下
                new_trans.translation.z -= 0.1; 
                // new_trans.translation.y += 0.3;
                
                ent.insert(InfantryViewOffset(new_trans));
            }
            _ => {}
        }
    }
