...
namespace rm_auto_aim   // 命名空间
{
Detector::Detector( // 构造函数
...
std::vector<Armor> Detector::detect(const cv::Mat & input)
{
  ...
  //std::cout <<"qian"<<armors_.size() << std::endl;
  // if (!armors_.empty()) {
  //   classifier->extractNumbers(input, armors_);
  //   classifier->classify(armors_);
  // }
    //std::cout <<"hou"<<armors_.size() << std::endl;
  return armors_;
}

cv::Mat Detector::getAllNumbersImage()  // 用于显示数字
{
  if (armors_.empty()) {
    return cv::Mat(cv::Size(20, 28), CV_8UC1);  // 20*28的黑色图像
  } else {
